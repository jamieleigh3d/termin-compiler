...
| **Deploy-template generator emits v0.8-shape channel bindings** | journal 2026-04-29 cleanup-pass | `util/release.py` regenerates `<example>.deploy.json` files in the conformance fixtures dir, but the channel-binding template still emits the v0.8 top-level `channels` shape (URL + protocol + auth) rather than the v0.9 `bindings.channels.<name>.provider` + config shape. The v0.9 strict channel validator rejects the result, breaking ~110 conformance tests when fixtures are regenerated. Workaround in v0.9: keep using the existing committed deploy.json files; don't regenerate them via the release script. Real fix: update the deploy-template generator in `termin/cli.py` (or wherever it lives) to emit `provider: "stub"` + matching config for each declared channel. v0.9 maintenance / pre-v0.9.0-tag cleanup. |
| **Drop `duration_ms` kwarg back-compat in `write_audit_trace`** | technical-debt note above; journal 2026-04-29 cleanup-pass | The kwarg accepts both `duration_ms=` (legacy) and `latency_ms=` (BRD §6.3.4). Internal call sites at `compute_runner.py:1090` and `:1159` still pass `duration_ms=`, plus 2 test files. Migrating those call sites + dropping the kwarg + removing the explicit "legacy kwarg still works" test in `test_v09_compute_audit.py` is a clean ~half-hour slice — not done in the v0.9 cleanup pass because the prerequisite ("no remaining call sites") wasn't actually met (the workspace MEMORY assumed it was). Left intact for v0.9 ship; drop in maintenance. |
