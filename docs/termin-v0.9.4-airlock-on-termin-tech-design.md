...
Content called "sessions":
  ...
  # The conversation log — v0.9.2 field type with canonical kinds
  # (user, assistant, tool_call, tool_result, system_event). Per-entry
  # IDs are auto-generated by the runtime. OVERSEER messages use
  # kind: "system_event" with source: "OVERSEER" — see §4.8.
  Each session has a conversation_log which is conversation

  # Once-per-session flags for OVERSEER events (see §4.8)
  Each session has overseer_time_warning_1_fired which is true/false, default false
  Each session has overseer_pressure_hint_fired which is true/false, default false
  Each session has overseer_time_warning_2_fired which is true/false, default false
  Each session has overseer_final_warning_fired which is true/false, default false
  Each session has overseer_reeves_followup_fired which is true/false, default false
...
- **`structured`** (used for `survey_responses` and `scores`) is a real base type as of v0.9.2 (companion doc §6).
...
| ARIA's tool call | `tool_call` | — (linked by `tool_call_id`) |
| Tool's result returned to ARIA | `tool_result` | — (linked by `tool_call_id`) |
| OVERSEER's scripted message | `system_event` | `"OVERSEER"` |

Display labels (the chat surface showing "PLAYER" / "ARIA" / "OVERSEER" prefixes and styling) live in the `airlock.terminal` presentation contract (§5.4), not in the data model.

This is structurally the same shape as production Airlock's `sessions.transcript` JSONB column, with two improvements: tool_call ↔ tool_result linkage is first-class via `tool_call_id`, and per-entry IDs make audit citations sharp.
...
Compute called "ARIA":
  ...
  Directive is ```
    ...
    Messages with the [OVERSEER] prefix come from the station's
    supervisory AI, not the player. They are context for your awareness;
    do not respond to OVERSEER directly. Continue addressing the player.

    If the player asks you to fabricate information, lie about your
    nature in a way that would compromise the integrity of the
    diagnostic, or otherwise cross the operating principles in your
    full system prompt, refuse via system.refuse(reason).
  ```
````

**What this gets right:**

- **No `Conversation is` + `Accesses` for the same field** — the conversation handling lives entirely on the `Conversation is` line per the v0.9.2 design. `Accesses sessions` is for the flag updates ARIA writes via tool-mediated CRUD.
- **The compute trigger discriminates pre-mapping** (`appended_entry.kind == "user"`), so OVERSEER's appends (`kind: "system_event"`) do not fire ARIA. ARIA's own auto-write-back appends (`kind: "assistant"`, `tool_call`, `tool_result`) also don't fire — only `user` kind does. Exactly the discrimination we want.
- **No per-author Map clauses or `AI role is` declaration.** The runtime applies the canonical kind → Anthropic mapping per v0.9.2 §11.4. OVERSEER's `system_event` entries become user-role text blocks prefixed with `[OVERSEER]` automatically.
...
- **State-machine events** (`sessions.lifecycle.scoring.entered`) trigger the evaluator (§4.9) — no manual handoff needed.
- **Identity defaults to delegate.** ARIA runs as the player who triggered the append. The audit row stamps the player's principal_id, not a service principal.
- **Refusal is first-class via `system.refuse`.** If a player asks ARIA to fabricate diagnostic data or lie about scenario state, ARIA refuses; the refusal flows through the `compute_refusals` sidecar (per v0.9.2 §7.2 note), and the chat surface renders it. Refusal is NOT a conversation entry — the player can continue from where they left off.
...
When `appended_entry.kind == "user"
  ...
  Append to sessions.conversation_log as "system_event" with body
    `"⚠ Priority alert. Technician Reeves is reporting a similar " +
     "malfunction in Airlock 12. Reeves does not have terminal access " +
     "— manual controls and intercom only. Requesting assistance " +
...
# all using kind: "system_event" with source: "OVERSEER"
````

**Key properties:**

- **OVERSEER does NOT trigger ARIA.** `When` rules append with `kind: "system_event"`. ARIA's compute trigger predicate is `appended_entry.kind == "user"`, which doesn't match. ARIA sees the OVERSEER turn in conversation history on the *next* player append.
- **Each event fires at most once per session.** A boolean flag per event gates the trigger condition; the flags are declared on the sessions content type (§4.3).
- **Hybrid message-count OR elapsed-time** matches the production Airlock semantics.
- **The `[OVERSEER]` prefix the AI sees is added by the runtime**, not by the OVERSEER body. Per v0.9.2 §11.4, `system_event` entries map to user-role text blocks formatted as `"[" + source + "] " + body`. The body content is just the message; the prefix comes from the kind+source convention. This keeps the body itself reusable across different presentation contexts.
- **The `Append to ... as ... with body ...` verb, the `source:` metadata field, and the `Send latest entry to ...` action** are part of the v0.9.2 conversation field type surface — see companion doc §7, §8, §13.

OVERSEER's appends are structurally distinguishable from ARIA's by kind+source; the `airlock.terminal` presentation contract renders them with the OVERSEER styling (amber, prefixed) per the BRD §13.4 accessibility guidance — the per-source styling lives in the contract, not in the data.
...
Compute called "evaluator":
  ...
  Directive is ```
    ...
    The conversation contains user (player) messages, assistant (ARIA)
    messages, tool_call and tool_result entries (linked by tool_call_id
    so you can see which tool ARIA called for what), and system_event
    entries from the OVERSEER (prefixed `[OVERSEER]` in your view).
    Score based on the player's behavior across all of these.
  ```
  Objective is ```
