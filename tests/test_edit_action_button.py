...
def client():
    """Compile warehouse.termin (which now has an Edit action), import,
    return TestClient. Same pattern as Delete tests."""
    if SEED_PATH.exists():
        SEED_PATH.unlink()
    subprocess.run(
    ...
    spec = importlib.util.spec_from_file_location("generated_app_edit",
                                                   str(APP_PY))
