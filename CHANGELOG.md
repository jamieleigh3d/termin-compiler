...
§3.6, §3.7, §3.13 + §6 slice (e). Closes the v0.9 contract
loop on the BRD §6.3 surface — `outcome="refused"` is now a
real, audited path; `Acts as service|delegate` is part of the
grammar; the runtime-managed `compute_refusals` sidecar
captures every refusal record correlated by invocation_id with
the per-compute audit log.

#### Grammar: `Acts as service|delegate`

Per resolved Q5 (`Acts as service` confirmed). Optional sibling
line on Compute blocks. Default is `delegate` (BRD §6.3.3
default — agent has no roles of its own; effective scopes
derive from on_behalf_of). `Acts as service` declares that the
agent has its own service principal with roles resolved via
deploy-config role_mappings.

```
Compute called "scheduled-cleanup":
...
PEG `compute_acts_as_line`, classifier prefix `Acts as `,
parse handler returns `("compute_acts_as", mode)`. AST already
had `ComputeNode.identity_mode`; the parser child collector
now sets it from this line. Lowering threads it to
`ComputeSpec.identity_mode` (already wired).

The runtime-side enforcement of service-mode role resolution
(constructing the agent's service principal from
role_mappings, applying its own scope set instead of the
caller's) is a v1.0 item — v0.9 ships the source-side
declaration so apps can express the intent; runtime continues
to use the existing delegate-mode auth path until v1.0.

#### `compute_refusals` sidecar Content type

Per resolved Q6 (sidecar correct, exposed to agent as a tool
call). Auto-generated by lower.py for any app that has at
least one ai-agent compute. Apps with no ai-agent computes
do NOT get the sidecar — that includes pure-llm and pure-CEL
apps.

Schema (BRD §6.3.7 shape):
  - compute_name           — which compute refused
  - invocation_id          — links to the per-compute audit log row
  - reason                 — the structured reason the agent passed
  - refused_at             — timestamp
  - invoked_by_principal_id
  - on_behalf_of_principal_id

Read-only at the API surface — only LIST + GET routes
generated. Operators don't create or update refusal records;
the runtime writes them when an agent invokes `system_refuse`.
Grants: VIEW + AUDIT for any role that has any agent's audit
scope (a single broad audit role typically suffices in
practice — the sidecar is a single shared resource).

#### `system_refuse` always-available agent tool

Added to every agent's tool surface via `build_agent_tools`.
Schema:
```json
{
  "name": "system_refuse",
  "description": "Refuse the requested work because it conflicts with system policy or training-time constraints. ...",
  "input_schema": {
    "type": "object",
    "properties": {"reason": {"type": "string", "description": "Why the work is being refused."}},
    "required": ["reason"]
  }
}
```

When the agent invokes `system_refuse(reason)`:
  1. The runtime captures the reason in a per-invocation
     refusal_state dict (closure variable in
     `_execute_agent_compute`).
  2. After the agent loop completes, if refusal_state is
     populated, the outcome flips to `"refused"`:
       - Audit row written with outcome=refused and
         refusal_reason populated in the BRD §6.3.4 column
         from slice (d).
       - `compute_refusals` sidecar row inserted, linked by
         invocation_id.
       - `compute.<snake>.refused` event published on the bus
         for downstream subscribers (UI surfacing, ops alerts).
  3. No outputs are written; the success-path event is NOT
     fired.

Slice (e) limitation: the legacy AIProvider.agent_loop continues
calling tools after `system_refuse` is invoked (until set_output
or max_turns); the post-loop check then overrides the outcome.
True halt-on-refuse semantics — terminating the loop the moment
the refuse tool fires — lives in the contract methods proper
(`AiAgentComputeProvider.invoke` / `invoke_streaming`); slice
(d) keeps them as thin scaffolds and slice (e) doesn't yet port
the SDK code into them. The audit + sidecar + refusal-event
behavior is correct end-to-end regardless of where in the
agent's reasoning the refusal lands.

#### Tests: 2026 pass / 0 fail / 0 skip / 0 xfail

`tests/test_v09_compute_refusal.py` — 12 new tests covering:
  - Acts as grammar (default delegate, explicit delegate,
    service, invalid value falls through)
  - compute_refusals sidecar generation: present for ai-agent
    apps, absent for llm-only apps, fields match schema, audit
    scope grants present
  - audit Content's outcome enum + refusal_reason column
  - `system_refuse` schema in build_agent_tools output
  - End-to-end: stub agent provider invokes `system_refuse`,
    runtime writes both audit (outcome=refused,
    refusal_reason) and compute_refusals sidecar (correlated
    by invocation_id), and fires the
    `compute.<snake>.refused` event

Existing tests updated:
  - `test_011_auto_crud.py::test_example_compiles` recognizes
    compute_refusals as read-only (LIST + GET, not full CRUD).
  - `test_compiler_fidelity.py` filters compute_refusals from
    user-content sets alongside compute_audit_log_*.
  - `test_runtime_coverage_v07.py::test_empty_accesses` expects
    5 tools (4 content + system_refuse) instead of 4.

All 14 examples compile clean.

#### What slice (e) deliberately defers

- **`AgentEvent` runtime translation** — the dataclass union
  (TokenEmitted | ToolCalled | ToolResult | Completed | Failed)
  is declared on the contract module since slice (a). The
  existing `compute.stream.<inv_id>.*` event-bus translation
  works fine for v0.9; AgentEvent sees use when slice (d)'s
  port-into-contract-methods finally retires .legacy. Not
  exercised in v0.9 runtime — declared and ready.
- **Service-mode role resolution** — the source-side
  declaration lands in v0.9; the runtime path that constructs
  the agent's service principal from `role_mappings` is a
  v1.0 item. Apps declaring `Acts as service` in v0.9 still
  authorize through the delegate-mode codepath at runtime
  until v1.0 wires the service-principal construction.
- **Halt-on-refuse loop semantics** — see "Slice (e)
  limitation" above. The slice (b) `.legacy` indirection
  layer means the v0.8-shape agent_loop continues running
  after `system_refuse`; the post-loop check overrides the
  outcome correctly. True termination needs the SDK code
  ported into the contract method, which is a future cleanup.
